...
class HeartbeatService:
    ...
    async def _log_heartbeat_stats(self):
        ...
        print(
            f"     • First: {stats['first_record'].strftime('%H:%M:%S') if stats['first_record'] else 'N/A'}"
        )
        print(
            f"     • Last: {stats['last_record'].strftime('%H:%M:%S') if stats['last_record'] else 'N/A'}"
        )
    ...
    async def _log_connection_info(self):
        ...
        """)
        print(
            f"   🔌 Connections: {pool_info['active_connections']}/{pool_info['max_connections']}"
        )
